...
uses
	Winapi.Windows, Classes, Vcl.StdCtrls, Vcl.ExtCtrls, Vcl.Controls,
  Winapi.Messages, SysUtils, Vcl.Forms, Vcl.Graphics, Vcl.Menus, Vcl.Buttons;
...
{ TExtSpinEdit }
  ...
  published
  	 property BtnCursor: TCursor read GetBtnCursor write SetBtnCursor default crDefault;
    property Anchors;
    property AutoSelect;
    property AutoSize;
    property Color;
...
begin
  	Result:=(Key in [FormatSettings.DecimalSeparator, '+', '-', '0'..'9']) or (Key < #32);
...
procedure TExtSpinEdit.CreateWnd;
begin
  inherited CreateWnd;
  SetEditRect;
end;

procedure TExtSpinEdit.SetEditRect;
var
  Loc: TRect;
begin
...
function TExtSpinEdit.GetMinHeight: Integer;
var
  DC: HDC;
  SaveFont: HFont;
  I: Integer;
...
begin
  	If ReadOnly then MessageBeep(0)
   	...
   	begin
         If (FMaxValue <> FMinValue) and
         	(Value = FMaxValue) then Value:=FMinValue
         else
...
begin
  	If ReadOnly then MessageBeep(0)
   	...
   	begin
         If (FMaxValue <> FMinValue) and
         	(Value = FMinValue) then Value:=FMaxValue
         else
   			Value:=Value - FIncrement;
      end;
end;
...
function TExtSpinEdit.GetValue: LongInt;
begin
  	Try
...
procedure TExtSpinEdit.SetValue(NewValue: LongInt);
begin
  	Text:=IntToStr(CheckValue(NewValue));
end;

function TExtSpinEdit.CheckValue(NewValue: LongInt): LongInt;
begin
  	Result:=NewValue;
...
procedure TExtSpinEdit.SetBtnCursor(const Value: TCursor);
begin
   FButton.FUpButton.Cursor:=Value;
   FButton.FDownButton.Cursor:=Value;
end;

function TExtSpinEdit.GetBtnCursor: TCursor;
begin
 	Result:=FButton.FUpButton.Cursor;
end;
